...
export default function QuoteForm({ tripSummary }: QuoteFormProps) {
  ...
  async function handleEmail() {
    ...
    try {
      const res = await fetch("https://formsubmit.co/ajax/servicioalcliente@agendatur.com", {
        method: "POST",
      ...
      window.location.href = `mailto:servicioalcliente@agendatur.com?subject=${encodeURIComponent(
    ...
    <div className="space-y-4">
      ...
      {emailStatus === "error" && (
        <p className="text-xs text-gray-500">
          Abrimos tu cliente de correo para que envíes la solicitud directamente a servicioalcliente@agendatur.com.
        </p>
      )}
    </div>
  );
